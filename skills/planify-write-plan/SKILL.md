...
description: "Rédacteur interne du skill planify : met en forme le récapitulatif d'un plan (produit par l'interrogatoire de planify) en deux fichiers dans .task/<KEY>/<prefix>/ — un plan.md lisible (tâches + contraintes d'acceptation) et un tech.md dense. Invoqué par planify via l'outil Skill ; ne se déclenche pas seul et n'a pas de slash command. Ne repose aucune question, ne prend aucune décision."
...
1. Utiliser le chemin `.task/<KEY>/<prefix>/` transmis par `planify` dans le récapitulatif.
2. Créer ce dossier s'il n'existe pas. Si le chemin manque, le signaler à `planify` sans le choisir.

**Reprise** : si `plan.md` existe déjà dans ce dossier, le mettre à jour avec `Edit` (fusionner les changements
du récapitulatif, préserver le reste) plutôt que de l'écraser aveuglément.

## `plan.md` — lisible, facile à relire
...
## `tech.md` — index technique dense pour l'agent
